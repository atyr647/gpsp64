...
def emit_dispatch(out, entry_to_func):
    """Emit the PC -> translated-function lookup.

    This runs at the top of the Thumb interpreter loop, i.e. once per
    interpreted instruction, so its cost is paid even when the PC is not
    an AOT entry point.  A `switch (pc)` over ~600 sparse 32-bit labels
    compiles to a binary search -- ~10 compare-and-branch pairs, every
    one of them unpredictable on the VR4300 (which has no branch
    predictor worth the name).  Since the hot AOT'd pages are also the
    pages where the interpreter spends most of its time, that search was
    being run on the majority of interpreted instructions.

    Replace it with a two-level table:

      aot_page_tab[(pc >> 12) & 0x1FFF]   -> {fns, slots} for that 4KB page
      slots[(pc & 0xFFF) >> 1]            -> 1-based index into fns, 0 = not an entry

    Two dependent loads and two not-taken branches, no search.  The page
    table is a flat 8192 entries so the mask alone bounds it (no compare),
    costing 64KB of .rodata; the slot maps cost 2KB per covered page.
    That is a deliberate memory-for-speed trade -- it is affordable
    ...
    pages = {}          # page_idx -> {'fns': [fn_addr], 'index': {fn_addr: slot}, 'slots': {halfword: slot}}
    for ep in sorted(entry_to_func):
        fn = entry_to_func[ep]
        pidx = (ep >> 12) & 0x1FFF
        pg = pages.setdefault(pidx, {'fns': [], 'index': {}, 'slots': {}})
        if fn not in pg['index']:
            pg['index'][fn] = len(pg['fns']) + 1   # 1-based; 0 means "no entry"
            pg['fns'].append(fn)
        pg['slots'][(ep & 0xFFF) >> 1] = pg['index'][fn]

    for pidx, pg in sorted(pages.items()):
        if len(pg['fns']) > 255:
            raise SystemExit(
                f'page 0x{pidx:04X} has {len(pg["fns"])} translated functions, '
                'but slot maps are u8 (max 255).  Split the range in '
                'n64/aot_targets.txt or widen aot_slot_* to u16.')

    out.write('\n/* ---- PC dispatch tables (see emit_dispatch in tools/thumb2c.py) ---- */\n')
    for pidx, pg in sorted(pages.items()):
        out.write(f'\nstatic const aot_fn_t aot_fns_{pidx:04X}[] = {{\n')
        for fn in pg['fns']:
            out.write(f'    aot_gen_{fn:08X},\n')
        out.write('};\n')
        out.write(f'static const u8 aot_slots_{pidx:04X}[2048] = {{\n')
        for hw in sorted(pg['slots']):
            out.write(f'    [0x{hw:03X}] = {pg["slots"][hw]},\n')
        out.write('};\n')
    ...
    out.write('const struct aot_page_ent aot_page_tab[8192] = {\n')
    for pidx, _ in sorted(pages.items()):
    ...
    out.write('#endif\n')
    out.write('    {\n')
    out.write('        const struct aot_page_ent *p = &aot_page_tab[(pc >> 12) & 0x1FFF];\n')
    out.write('        u32 slot;\n')
    out.write('        if (!p->slots) return 0;\n')
    out.write('        slot = p->slots[(pc & 0xFFFu) >> 1];\n')
    out.write('        if (!slot) return 0;\n')
    out.write('        *cycles_used = p->fns[slot - 1](pc);\n')
    out.write('        return 1;\n')
    out.write('    }\n')
    out.write('}\n')
