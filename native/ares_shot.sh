...
( cd "$SCRATCH" && DISPLAY="$DISP" timeout $((WAIT + 30)) \
    "$ARES" --kiosk \
      --setting Video/WindowWidth=1280 --setting Video/WindowHeight=960 \
      --setting Video/Multiplier=4 --setting Video/Output=center \
      --setting Video/AspectCorrection=false --setting Video/Overscan=false \
      --setting Nintendo64/ExpansionPak=true \
      --system "Nintendo 64" gpsp.z64 ) \
    >"$SCRATCH/ares.log" 2>&1 & APID=$!
sleep "$WAIT"
# X sees whatever ares has painted at that instant, which is regularly a
# half-updated frame -- at 15 fps against a 60 Hz refresh most captures
# catch the framebuffer mid-write.  Take several and keep the one with
# the most distinct colours, which is the most complete one.
BEST=0
for i in 1 2 3 4 5 6; do
  DISPLAY="$DISP" import -window root "/tmp/shot_$i.png" 2>/dev/null || continue
  N=$(convert "/tmp/shot_$i.png" -format "%k" info: 2>/dev/null); N=${N:-0}
  if [ "$N" -gt "$BEST" ]; then BEST=$N; cp "/tmp/shot_$i.png" "$OUT"; fi
  sleep 1
done
echo "[shot:$LABEL] best of 6 captures: $BEST colours"
